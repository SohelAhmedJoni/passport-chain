...
function generateCCAASCerts() {
  ...
  openssl req -new -x509 -nodes -key $cc_certs_base_path/ca/key.pem -out $cc_certs_base_path/ca/cert.pem -subj "/C=US/ST=California/L=San Francisco/O=${org}.example.com Inc/OU=Developer/CN=passport-chain.${org}.example.com"

  echo subjectAltName=DNS:passport-chain.${org}.example.com > $cc_certs_base_path/subj.cnf

  # CC Server certs
  infoln "Creating CC Server certs"
  mkdir -p $cc_certs_base_path/server
  openssl req -newkey rsa:2048 -nodes -keyout $cc_certs_base_path/server/key.pem -out $cc_certs_base_path/server/req.pem -subj "/C=US/ST=California/L=San Francisco/O=${org}.example.com Inc/OU=Developer/CN=passport-chain.${org}.example.com"
