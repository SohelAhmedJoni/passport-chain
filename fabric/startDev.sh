...
docker network create passport-chain-net
./network.sh up createChannel -n $ORG_QNTY $CCAAS_TLS_ENABLED

if [ "$DEPLOY_CCAAS" = "false" ]; then
  ./network.sh deployCC -ccn passport-chain -ccp ../chaincode -ccl go -n $ORG_QNTY -cccg $CCCG_PATH
else
  ./network.sh deployCCAAS -ccn passport-chain -ccp ../chaincode -n $ORG_QNTY -cccg $CCCG_PATH $CCAAS_TLS_ENABLED
fi
