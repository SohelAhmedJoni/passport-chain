...
if [ "$DEPLOY_CCAAS" = "false" ]; then
  ./network.sh deployCC -ccn passport-chain -ccp ../chaincode -ccl go -ccv $version -ccs $sequence -n $ORG_QNTY -cccg $CCCG_PATH -cci init
else
  ./network.sh deployCCAAS -ccn passport-chain -ccp ../chaincode -ccv $version -ccs $sequence -n $ORG_QNTY -cccg $CCCG_PATH
fi
cd ..
